...
object CreateTrain {
  ...
  def saveDfByWeek(spark: SparkSession
                   , goodsale: DataFrame, goods_promote_price: DataFrame, goodsdaily: DataFrame,
                   weekNum: Int) = {
    import spark.implicits._
    val goodsale_60 = goodsale.filter($"weekNum" === weekNum)
    val goods_promote_price_60 = goods_promote_price.filter($"weekNum" === weekNum)
    val goodsdaily_60 = goodsdaily.filter($"weekNum" === weekNum)
    ...
    goodsale_60
      ...
      .drop("weekNum")
      .write.mode("overwrite").parquet("output/train_" + weekNum)
    println("保存成功：output/train_" + weekNum)
  }
  ...
  def main(args: Array[String]): Unit = {
    ...
    //每日操作记录表
    goodsdaily = goodsdaily.withColumn("weekNum", udf_date2weekNum(col("data_date")))
    ...
    /**
      ...
      *
      */
//    saveDfByWeek(spark, goodsale, goods_promote_price, goodsdaily, 52)
//    saveDfByWeek(spark, goodsale, goods_promote_price, goodsdaily, 51)
//    saveDfByWeek(spark, goodsale, goods_promote_price, goodsdaily, 50)
//    saveDfByWeek(spark, goodsale, goods_promote_price, goodsdaily, 49)
//    saveDfByWeek(spark, goodsale, goods_promote_price, goodsdaily, 48)
    saveDfByWeek(spark, goodsale, goods_promote_price, goodsdaily, 7)
