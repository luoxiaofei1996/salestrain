  ...
  def statisticDf(
    ...
    df
      ...
      .map(x => {
        ...
        val value = collection.mutable.Map(x.get(x.fieldIndex(timeCol)).toString.toInt -> array.toArray)
        (key, value)
      })
      .combineByKey(
        (v) => v,
        (acc: collection.mutable.Map[Int, Array[Double]], v: collection.mutable.Map[Int, Array[Double]]) => acc ++ v,
        (acc1: collection.mutable.Map[Int, Array[Double]], acc2: collection.mutable.Map[Int, Array[Double]]) => acc1 ++ acc2
      )
      .map(x => {
        //id
        val id = x._1
